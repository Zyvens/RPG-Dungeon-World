...
;(() => {
 ...
 #dynamicPeopleGrid .person-fields{display:grid!important;grid-template-columns:140px minmax(220px,1fr) minmax(180px,.65fr)!important;gap:10px 12px!important;width:100%!important;align-items:start}
 .char-photo{grid-row:1/3;width:140px;height:170px;border:1px dashed rgba(193,154,91,.45);border-radius:12px;overflow:hidden;background:rgba(4,13,22,.55);display:flex;align-items:center;justify-content:center;position:relative}
 .char-photo img{width:100%;height:100%;object-fit:cover;display:block}.char-photo.empty img{display:none}.char-photo-placeholder{text-align:center;padding:12px;color:var(--muted);font-size:.78rem;line-height:1.35}.char-photo:not(.empty) .char-photo-placeholder{display:none}
...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const u=new URL(e.request.url);if(u.pathname.endsWith("/app.js")){e.respondWith(caches.match(e.request).then(async cached=>{let r=cached;if(!r){try{r=await fetch(e.request);if(r&&r.ok&&r.type==="basic"){const cl=r.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,cl))}}catch(_){}}if(!r)return new Response("",{status:503});const src=await r.clone().text();return new Response(src+UI_PATCH,{status:r.status,statusText:r.statusText,headers:{"Content-Type":"application/javascript; charset=utf-8","Cache-Control":"no-cache"}})}));return}e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(r=>{if(r&&r.ok&&r.type==="basic"){const cl=r.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,cl))}return r}).catch(()=>cached))) });