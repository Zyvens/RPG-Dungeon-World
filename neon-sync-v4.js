(() => {
  ...
  async function remote(){const {data,error}=await client.from('kael_app_state').select('user_id,payload,version,updated_at').limit(1);if(error)throw error;return Array.isArray(data)&&data.length?data[0]:null}
  async function createRemote(payload){const {data,error}=await client.from('kael_app_state').insert({payload}).select('version,updated_at');if(error)throw error;return Array.isArray(data)?data[0]:data}
  async function updateRemote(payload,base){const {data,error}=await client.from('kael_app_state').update({payload,version:Number(base||0)+1,updated_at:new Date().toISOString()}).eq('user_id',user.id).select('version,updated_at');if(error)throw error;return Array.isArray(data)?data[0]:data}
  ...
  async function applyRemote(row){
    if(!row?.payload||editing())return;
    busy=true;
    try{
      rememberView();
      await idbSet(row.payload.idbState);
      const rm=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(isAppKey(k))rm.push(k)}rm.forEach(k=>localStorage.removeItem(k));
      Object.entries(row.payload.localStorage||{}).forEach(([k,v])=>{if(isAppKey(k)&&v!=null)localStorage.setItem(k,String(v))});
      meta(row.version,hash(row.payload));
      location.reload();
    }finally{busy=false}
  }

  async function uploadSnapshot({allowEditing=false,label='Sincronizado'}={}){
    if(!user||busy||!navigator.onLine)return false;
    if(editing()&&!allowEditing)return false;
    busy=true;
    try{
      status('Salvando na nuvem…','sync');
      const local=await snapshot(),h=hash(local),r=await remote();
      const out=r?await updateRemote(local,Math.max(lastVersion,Number(r.version||0))):await createRemote(local);
      meta(out?.version||Number(r?.version||0)+1,h);
      gameplayDirty=false;
      status(label,'ok');
      return true;
    }catch(e){console.error(e);status('Erro ao salvar na nuvem','error');return false}
    finally{busy=false}
  }
  async function uploadSaved(){return uploadSnapshot({allowEditing:false,label:'Sincronizado ao salvar'})}
  async function uploadGameplay(){
    gameplayTimer=null;
    if(!gameplayDirty||!gameplayActive())return;
    if(busy){scheduleGameplaySync();return}
    const ok=await uploadSnapshot({allowEditing:true,label:'Gameplay sincronizado'});
    if(!ok&&gameplayDirty)scheduleGameplaySync();
  }
  function scheduleGameplaySync(){
    gameplayDirty=true;
    clearTimeout(gameplayTimer);
    gameplayTimer=setTimeout(uploadGameplay,GAMEPLAY_DELAY);
    if(user)status('Gameplay salvo localmente…','sync');
  }

  /* O Gameplay já persiste cada alteração em localStorage. Interceptamos somente
     suas chaves para agendar uma sincronização silenciosa; nenhuma escrita é bloqueada. */
  const nativeSetItem=Storage.prototype.setItem;
  Storage.prototype.setItem=function(key,value){
    const result=nativeSetItem.call(this,key,value);
    if(this===localStorage&&GAMEPLAY_KEYS.has(String(key)))scheduleGameplaySync();
    return result;
  };

  function style(){if(document.getElementById('neon-v4-style'))return;const s=document.createElement('style');s.id='neon-v4-style';s.textContent=`
  ...
  async function openSync(){if(editing()&&!gameplayActive()){status('Edição local — sincroniza ao salvar','sync');return}document.getElementById('neonSyncError').textContent='';document.getElementById('neonSyncModal')?.classList.add('open');const m=document.getElementById('neonRemoteMeta');m.textContent='Consultando versão da nuvem…';try{const r=await remote();m.textContent=r?`Última versão na nuvem: ${new Date(r.updated_at).toLocaleString('pt-BR')}`:'Ainda não há conteúdo salvo na nuvem.'}catch{m.textContent='Não foi possível consultar a nuvem agora.'}}
  async function authDo(mode){const email=document.getElementById('neonEmail').value.trim(),password=document.getElementById('neonPassword').value||'',err=document.getElementById('neonAuthError');if(!email||password.length<8){err.textContent='Informe um e-mail válido e uma senha com pelo menos 8 caracteres.';return}err.textContent='';try{const r=mode==='up'?await client.auth.signUp.email({email,password,name:email.split('@')[0]||'Kael'}):await client.auth.signIn.email({email,password});if(r?.error)throw r.error;const s=sessionOf(await client.auth.getSession());if(!s?.user){err.textContent='Conta criada. Se houver verificação de e-mail, confirme e depois entre.';return}user=s.user;close('neonAuthModal');status('Conectado — Gameplay sincroniza automaticamente','ok')}catch(e){err.textContent=e?.message||'Falha na autenticação.'}}
  ...
  async function pushFlow(){if(busy||editing())return;const b=document.getElementById('neonPush'),err=document.getElementById('neonSyncError');const r=await remote().catch(()=>null);if(r&&b.dataset.confirm!=='1'){b.dataset.confirm='1';b.textContent='Confirmar: substituir a nuvem';err.textContent='Já existe conteúdo na nuvem. Clique novamente para confirmar que este dispositivo é a versão correta.';return}b.dataset.confirm='';b.textContent='Enviar para a nuvem';err.textContent='';await uploadSaved();close('neonSyncModal')}
  async function signOut(){await client.auth.signOut();user=null;localStorage.removeItem(VER);localStorage.removeItem(HASH);lastVersion=0;lastHash='';close('neonSyncModal');status('Entrar para sincronizar')}
  function bindSave(){const b=document.getElementById('saveBtn');if(!b||b.dataset.neonV4)return;b.dataset.neonV4='1';b.addEventListener('click',()=>{clearTimeout(gameplayTimer);gameplayTimer=null;status('Aguardando salvamento local…','sync');setTimeout(()=>uploadSaved(),600)})}
  async function boot(){inject();bindSave();restoreView();status('Carregando Neon…','sync');try{const sdk=await import(SDK_URL);client=sdk.createClient({auth:{url:AUTH_URL},dataApi:{url:DATA_API_URL}});const s=sessionOf(await client.auth.getSession());user=s?.user||null;status(user?'Conectado — Gameplay sincroniza automaticamente':'Entrar para sincronizar',user?'ok':'');if(user&&gameplayDirty)scheduleGameplaySync()}catch(e){console.error(e);status('Neon offline','error')}}
