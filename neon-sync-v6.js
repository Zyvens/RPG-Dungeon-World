(() => {
  ...
  // Keep Auth + Data API on the current Neon SDK line. 0.6.2-beta could expose
  // a Better Auth user before the JWT cache was ready, which made iOS PWAs look
  // connected while Data API calls still failed with AuthRequiredError.
  ...
  const REMEMBER=META+'remember', LAST_EMAIL=META+'email';
  const GAMEPLAY_KEYS=new Set(['kael-gameplay-maps-v1','kael-gameplay-active-v1','kael-turn-order-v1']);
  const GAMEPLAY_DELAY=8000;
  let client=null,user=null,busy=false,lastVersion=Number(localStorage.getItem(VER)||0),lastHash=localStorage.getItem(HASH)||'',gameplayTimer=null,gameplayDirty=false;
  ...
  async function resolveSession(authResult=null,{requireJwt=false}={}){
    const immediate=sessionOf(authResult);
    if(immediate?.user&&(!requireJwt||hasJwt(immediate)))return immediate;
    // iOS standalone WebKit can expose the newly-written cookie/cache a little
    // later than Safari. Retry long enough to bridge that handoff.
    for(const delay of [0,120,300,650,1200]){
      if(delay)await wait(delay);
      try{
        const s=sessionOf(await client.auth.getSession());
        if(s?.user&&(!requireJwt||hasJwt(s)))return s;
      }catch(e){console.warn('[Kael Neon] getSession retry failed',e)}
    }
    return null;
  }
  async function requireCloudSession(){
    const s=await resolveSession(null,{requireJwt:true});
    if(!s?.user){
      user=null;
      status('Sessão expirada — entre novamente','error');
      throw new Error('Sua sessão de sincronização não está disponível neste app. Entre novamente.');
    }
    user=s.user;
    return s;
  }

  async function remote(){await requireCloudSession();const {data,error}=await client.from('kael_app_state').select('user_id,payload,version,updated_at').limit(1);if(error)throw error;return Array.isArray(data)&&data.length?data[0]:null}
  async function createRemote(payload){await requireCloudSession();const {data,error}=await client.from('kael_app_state').insert({payload}).select('version,updated_at');if(error)throw error;return Array.isArray(data)?data[0]:data}
  async function updateRemote(payload,base){await requireCloudSession();const {data,error}=await client.from('kael_app_state').update({payload,version:Number(base||0)+1,updated_at:new Date().toISOString()}).eq('user_id',user.id).select('version,updated_at');if(error)throw error;return Array.isArray(data)?data[0]:data}
  ...
  async function uploadSnapshot({allowEditing=false,label='Sincronizado'}={}){if(!user||busy||!navigator.onLine)return false;if(editing()&&!allowEditing)return false;busy=true;try{status('Salvando na nuvem…','sync');await requireCloudSession();const local=await snapshot(),h=hash(local),r=await remote();const out=r?await updateRemote(local,Math.max(lastVersion,Number(r.version||0))):await createRemote(local);meta(out?.version||Number(r?.version||0)+1,h);gameplayDirty=false;status(label,'ok');return true}catch(e){console.error(e);status('Erro ao salvar na nuvem','error');return false}finally{busy=false}}
  async function uploadSaved(){return uploadSnapshot({allowEditing:false,label:'Sincronizado ao salvar'})}
  ...
  function inject(){style();if(document.getElementById('neonCloudStatus'))return;const chip=document.createElement('button');chip.id='neonCloudStatus';chip.className='neon-cloud-chip';chip.type='button';chip.textContent='☁ Entrar para sincronizar';chip.onclick=()=>user?openSync():openAuth();document.body.appendChild(chip);
    const auth=document.createElement('div');auth.id='neonAuthModal';auth.className='neon-modal';auth.innerHTML=`<div class="neon-box"><h3>Conta de sincronização</h3><p>Use a mesma conta no celular e no PC.</p><input id="neonEmail" type="email" autocomplete="username" placeholder="E-mail"><input id="neonPassword" type="password" autocomplete="current-password" placeholder="Senha (mínimo 8 caracteres)"><label class="neon-remember"><input id="neonRemember" type="checkbox" checked> Manter conectado neste dispositivo</label><p id="neonAuthError" class="neon-error"></p><div class="neon-actions"><button id="neonSignIn" class="primary">Entrar</button><button id="neonSignUp">Criar conta</button><button id="neonAuthClose">Fechar</button></div></div>`;document.body.appendChild(auth);auth.onclick=e=>{if(e.target===auth)close('neonAuthModal')};document.getElementById('neonAuthClose').onclick=()=>close('neonAuthModal');document.getElementById('neonSignIn').onclick=()=>authDo('in');document.getElementById('neonSignUp').onclick=()=>authDo('up');
    const remembered=localStorage.getItem(REMEMBER)!=='0';document.getElementById('neonRemember').checked=remembered;document.getElementById('neonEmail').value=remembered?(localStorage.getItem(LAST_EMAIL)||''):'';
    const sync=document.createElement('div');sync.id='neonSyncModal';sync.className='neon-modal';sync.innerHTML=`<div class="neon-box"><h3>Sincronizar dados</h3><p>Escolha claramente qual conteúdo deve prevalecer.</p><p class="neon-warning"><b>Nenhuma substituição acontece automaticamente.</b> No Gameplay, alterações locais são enviadas silenciosamente após alguns segundos; trazer conteúdo continua sendo uma decisão manual.</p><div class="neon-choice-grid"><div class="neon-choice in"><b>↓ Trazer conteúdo externo</b><small>Neon → este dispositivo. Use quando outro dispositivo contém a versão mais atual.</small><button id="neonPull">Trazer da nuvem</button></div><div class="neon-choice out"><b>↑ Exportar conteúdo atual</b><small>Este dispositivo → Neon. Use quando o conteúdo aberto aqui é o correto.</small><button id="neonPush">Enviar para a nuvem</button></div></div><p id="neonRemoteMeta" class="neon-meta"></p><p id="neonSyncError" class="neon-error"></p><div class="neon-actions"><button id="neonSyncClose">Fechar sem alterar nada</button><button id="neonSignOut" class="danger">Sair da conta</button></div></div>`;document.body.appendChild(sync);sync.onclick=e=>{if(e.target===sync)close('neonSyncModal')};document.getElementById('neonSyncClose').onclick=()=>close('neonSyncModal');document.getElementById('neonPull').onclick=pullFlow;document.getElementById('neonPush').onclick=pushFlow;document.getElementById('neonSignOut').onclick=signOut}
  function openAuth(){document.getElementById('neonAuthModal')?.classList.add('open')}
  async function openSync(){if(editing()&&!gameplayActive()){status('Edição local — sincroniza ao salvar','sync');return}const err=document.getElementById('neonSyncError');err.textContent='';document.getElementById('neonSyncModal')?.classList.add('open');const m=document.getElementById('neonRemoteMeta');m.textContent='Consultando versão da nuvem…';try{const r=await remote();m.textContent=r?`Última versão na nuvem: ${new Date(r.updated_at).toLocaleString('pt-BR')}`:'Ainda não há conteúdo salvo na nuvem.'}catch(e){m.textContent='Não foi possível consultar a nuvem agora.';err.textContent=e?.message||'A sessão da nuvem não está disponível.'}}
  async function authDo(mode){
    const email=document.getElementById('neonEmail').value.trim(),password=document.getElementById('neonPassword').value||'',err=document.getElementById('neonAuthError'),remember=document.getElementById('neonRemember').checked;
    if(!email||password.length<8){err.textContent='Informe um e-mail válido e uma senha com pelo menos 8 caracteres.';return}
    err.textContent='';status(mode==='up'?'Criando conta…':'Entrando…','sync');
    try{
      const r=mode==='up'?await client.auth.signUp.email({email,password,name:email.split('@')[0]||'Kael'}):await client.auth.signIn.email({email,password});
      if(r?.error)throw r.error;
      // A Data API needs the JWT, not only a Better Auth user object.
      const s=await resolveSession(r,{requireJwt:true});
      if(!s?.user){err.textContent='O login foi aceito, mas o token de sincronização ainda não ficou disponível neste app. Feche o app completamente, abra novamente e toque em Entrar.';status('Token ainda não disponível','error');return}
      user=s.user;
      localStorage.setItem(REMEMBER,remember?'1':'0');
      if(remember)localStorage.setItem(LAST_EMAIL,email);else localStorage.removeItem(LAST_EMAIL);
      close('neonAuthModal');status('Conectado — Gameplay sincroniza automaticamente','ok');
    }catch(e){console.error('[Kael Neon auth]',e);const msg=String(e?.message||'Falha na autenticação.');err.textContent=/invalid|credential|password/i.test(msg)?'E-mail ou senha inválidos.':msg;status('Falha no login','error')}
  }
  ...
  async function pushFlow(){if(busy||editing())return;const b=document.getElementById('neonPush'),err=document.getElementById('neonSyncError');let r=null;try{r=await remote()}catch(e){err.textContent=e?.message||'A sessão da nuvem não está disponível.';return}if(r&&b.dataset.confirm!=='1'){b.dataset.confirm='1';b.textContent='Confirmar: substituir a nuvem';err.textContent='Já existe conteúdo na nuvem. Clique novamente para confirmar a substituição.';return}b.dataset.confirm='';b.textContent='Enviar para a nuvem';err.textContent='';const ok=await uploadSnapshot({allowEditing:false,label:'Conteúdo enviado para a nuvem'});if(ok)close('neonSyncModal')}
  async function signOut(){try{await client.auth.signOut()}catch(e){console.warn(e)}user=null;localStorage.removeItem(LAST_EMAIL);localStorage.setItem(REMEMBER,'0');close('neonSyncModal');status('Entrar para sincronizar','')}
  function bindSave(){const b=document.getElementById('saveBtn');if(!b||b.dataset.neonV6)return;b.dataset.neonV6='1';b.addEventListener('click',()=>{clearTimeout(gameplayTimer);gameplayTimer=null;status('Aguardando salvamento local…','sync');setTimeout(()=>uploadSaved(),600)})}
  async function boot(){inject();bindSave();restoreView();status('Carregando Neon…','sync');try{const sdk=await import(SDK_URL);client=sdk.createClient({auth:{url:AUTH_URL},dataApi:{url:DATA_API_URL}});const remember=localStorage.getItem(REMEMBER)!=='0';if(!remember){try{await client.auth.signOut()}catch{}}const s=remember?await resolveSession(null,{requireJwt:true}):null;user=s?.user||null;status(user?'Conectado — Gameplay sincroniza automaticamente':'Entrar para sincronizar',user?'ok':'');if(user&&gameplayDirty)scheduleGameplaySync()}catch(e){console.error(e);status('Neon offline','error')}}
