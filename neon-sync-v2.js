(() => {
  ...
  const RESTORE_KEY = 'kael-neon-restore-view';
  ...
  function rememberView() {
    ...
    sessionStorage.setItem(RESTORE_KEY, JSON.stringify({ tab: active, x: scrollX, y: scrollY }));
  }
  ...
  function restoreView() {
    ...
    requestAnimationFrame(() => {
      if (saved.tab) document.querySelector(`.tab-btn[data-tab="${CSS.escape(saved.tab)}"]`)?.click();
      requestAnimationFrame(() => scrollTo(saved.x || 0, saved.y || 0));
    });
  }

  async function applyRemote(row) {
    if (!row?.payload || isEditing()) return false;
    busy = true;
    try {
      rememberView();
  ...
  async function uploadSavedState() {
    if (!currentUser || busy || !navigator.onLine || isEditing()) return;
    busy = true;
    try {
      ...
      const result = row
        ? await updateRemote(local, Math.max(lastVersion, Number(row.version || 0)))
        : await createRemote(local);
      setMeta(result?.version || Number(row?.version || 0) + 1, localHash);
      setStatus('Sincronizado ao salvar', 'ok');
    } catch (error) {
      console.error('[Kael Neon sync] save upload', error);
  ...
  async function initialSync({ manual = false } = {}) {
    if (!currentUser || busy || isEditing()) {
      if (isEditing()) setStatus('Edição local — sincroniza ao salvar', 'sync');
      return;
    }
    busy = true;
    try {
      setStatus('Verificando Neon…', 'sync');
      const local = await snapshot();
      const localHash = hash(local);
      const row = await remoteRow();
      ...
      if (manual && localHash !== lastHash) {
        busy = false;
        await uploadSavedState();
        return;
      }

      setMeta(Math.max(lastVersion, Number(row.version || 0)), localHash);
      setStatus('Sincronização em espera até salvar', 'ok');
    } catch (error) {
  ...
  function bindSaveSync() {
    const saveBtn = document.getElementById('saveBtn');
    if (!saveBtn || saveBtn.dataset.neonSyncBound === '1') return;
    saveBtn.dataset.neonSyncBound = '1';
    saveBtn.addEventListener('click', () => {
      setStatus('Aguardando salvamento local…', 'sync');
      setTimeout(() => uploadSavedState(), 500);
    });
  }
  ...
  function injectUI() {
    ...
    chip.id = 'neonCloudStatus';
    chip.className = 'neon-cloud-chip';
    chip.type = 'button';
    ...
    modal.id = 'neonAuthModal';
    modal.className = 'neon-auth-modal';
    modal.innerHTML = `<div class="neon-auth-box"><h3>Sincronização Neon</h3><p>Use a mesma conta no celular e no PC. Durante a edição nada é sincronizado; o envio ocorre ao clicar em Salvar e sair da edição.</p><input id="neonEmail" type="email" autocomplete="email" placeholder="E-mail"><input id="neonPassword" type="password" autocomplete="current-password" placeholder="Senha (mínimo 8 caracteres)"><p id="neonAuthError" class="neon-auth-error"></p><div class="neon-auth-actions"><button id="neonSignIn" class="primary" type="button">Entrar</button><button id="neonSignUp" type="button">Criar conta</button><button id="neonAuthClose" type="button">Cancelar</button></div><div id="neonUserLine" class="neon-user-line"></div></div>`;
  ...
  async function authenticate(mode) {
    ...
    try {
      const result = mode === 'signup'
        ? await client.auth.signUp.email({ email, password, name: email.split('@')[0] || 'Kael' })
  ...
  async function openAccount() {
    if (isEditing()) {
      setStatus('Edição local — sincroniza ao salvar', 'sync');
      return;
    }
    const choice = confirm(`Conectado como ${currentUser?.email || 'usuário'}.\n\nOK = sincronizar manualmente agora.\nCancelar = abrir opção de sair.`);
    if (choice) return initialSync({ manual: true });
